...
def show_movies():
    ...
    if sortMode == 'likes':
        movies = list(db.movies.find({'trashed': False}, {}).sort("like"))
    elif sortMode == 'viwers':
        movies = list(db.movies.find({'trashed': False}, {}).sort("num_viewers"))
    elif sortMode == 'date':
        movies = list(db.movies.find({'trashed': False}, {}).sort([("open_year", 1), ("open_month", 2), ("open_day", 3)]))
    else:
...
def show_movies():
    ...
    #    TODO: 다음 코드에서 likes로 정렬이 정상동작하도록 직접 수정해보세요!!!
    if sortMode == 'likes':
        movies = list(db.movies.find({'trashed': True}, {}).sort("like"))
    elif sortMode == 'viwers':
        movies = list(db.movies.find({'trashed': True}, {}).sort("num_viewers"))
    elif sortMode == 'date':
        movies = list(db.movies.find({'trashed': True}, {}).sort([("open_year", 1), ("open_month", 2), ("open_day", 3)]))
    else:
        return jsonify({'result': 'failure'})
...
def trash_movie():
    ...
    result = db.movies.update_one({'title':title_receive}, {'$set' : {'deleted': True}})
...
def restore_movie():
    ...
    result = db.movies.update_one({'title':title_receive}, {'$set' : {'deleted': false}})
...
def delete_movie():
    ...
    result = db.movies.find_one({'title':title_receive})
